#include "scripting/lua-bindings/auto/lua_creator_reader_auto.hpp"
...
int lua_creator_reader_PolygonCollider_getPoints(lua_State* tolua_S)
{
    int argc = 0;
    creator::PolygonCollider* cobj = nullptr;
    bool ok  = true;
...
#if COCOS2D_DEBUG >= 1
    if (!tolua_isusertype(tolua_S,1,"creator.PolygonCollider",0,&tolua_err)) goto tolua_lerror;
#endif

    cobj = (creator::PolygonCollider*)tolua_tousertype(tolua_S,1,0);

#if COCOS2D_DEBUG >= 1
    if (!cobj) 
    {
        tolua_error(tolua_S,"invalid 'cobj' in function 'lua_creator_reader_PolygonCollider_getPoints'", nullptr);
        return 0;
    }
#endif
    ...
    if (argc == 0) 
    {
        if(!ok)
        {
            tolua_error(tolua_S,"invalid arguments in function 'lua_creator_reader_PolygonCollider_getPoints'", nullptr);
            return 0;
        }
        std::vector<cocos2d::Vec2, std::allocator<cocos2d::Vec2> > ret = cobj->getPoints();
    ...
    luaL_error(tolua_S, "%s has wrong number of arguments: %d, was expecting %d \n", "creator.PolygonCollider:getPoints",argc, 0);
    return 0;

#if COCOS2D_DEBUG >= 1
    tolua_lerror:
    tolua_error(tolua_S,"#ferror in function 'lua_creator_reader_PolygonCollider_getPoints'.",&tolua_err);
#endif
...
int lua_register_creator_reader_PolygonCollider(lua_State* tolua_S)
{
    ...
    tolua_beginmodule(tolua_S,"PolygonCollider");
        tolua_function(tolua_S,"getPoints",lua_creator_reader_PolygonCollider_getPoints);
    tolua_endmodule(tolua_S);

...
Parallax1::Parallax1()
{
    ...
    cocosImage->setAnchorPoint( Vector2(0,0) );
    ...
    tilemap->setAnchorPoint( Vector2(0, 0) );
    ...
    voidNode->addChild(background, -1, Vector2(0.4f,0.5f), Vector2::ZERO);
    
    // tiles are moved at a ratio of 2.2x, 1.0y
    voidNode->addChild(tilemap, 1, Vector2(2.2f,1.0f), Vector2(0,-200) );
    
    // top image is moved at a ratio of 3.0x, 2.5y
    voidNode->addChild(cocosImage, 2, Vector2(3.0f,2.5f), Vector2(200,800) );
    
    
    // now create some actions that will move the 'void' node
    // and the children of the 'void' node will move at different
    // speed, thus, simulation the 3D environment
    auto goUp = MoveBy::create(4, Vector2(0,-500) );
    auto goDown = goUp->reverse();
    auto go = MoveBy::create(8, Vector2(-1000,0) );
...
Parallax2::Parallax2()
{
    ...
    voidNode->addChild(tilemap, 1, Vector2(1.0f,1.0f), Vector2(0,-200) );
    
    // top image is moved at a ratio of 3.0x, 2.5y
    voidNode->addChild( cocosImage, 2, Vector2(3.0f,2.5f), Vector2(200,1000) );
...
void Issue2572::update(float dt)
{
    ...
    if (_addTimer >= _addChildStep) {
        ...
        Vector2 offset = Vector2(viewSize.width / 2, viewSize.height/2);
        _paraNode->addChild(child, 1, Vector2( 1, 0 ), offset );

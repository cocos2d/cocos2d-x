...
function main() {
    ...
    for (nCntr = 0; nCntr < nNumWizards; nCntr++) {
        ...
        try {
            ...
            if (bDebug)
                WScript.Echo("Info: copy (" + strSrc + ") to (" + strDest + ")");
        ...
        try {
            ...
            if (e.description.length != 0)
                strError = e.description;
            WScript.Echo("ERROR: Cannot read and write CCAppWiz.vsz (" + strError + ")");
            return;
        }
    }
    ...
    try {
        ...
        if (bDebug)
            WScript.Echo("CC Folder: " + strDestCCFolder);
    }
    catch (e) {
        var strError = "no info";
        if (e.description.length != 0)
            strError = e.description;
        WScript.Echo("ERROR: Cannot create CC folder (" + strError + ")");
        return;
    }

    // Read and write additional CCAppWiz.vsdir and CCAppWiz.wophone.vsdir, add path to the wizard location
    try {
        ...
        if (e.description.length != 0)
            strError = e.description;
        WScript.Echo("ERROR: Cannot read and write CC\\CCAppWiz.vsdir (" + strError + ")");
        return;
    }

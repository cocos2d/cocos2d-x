...
private:
    static void enableAccelerometer() {
        if (accelerometer_g_user != nullptr) {
            OHOS_LOGI("Sensor Accelerometer is Listen");
            return;
        }
        ...
        OHOS_LOGI("Sensor Accelerometer is enable Listen");
    }
    
    static void disableAccelerometer() {
        if (accelerometer_g_user == nullptr) {
            OHOS_LOGI("Sensor Accelerometer is not Listen");
            return;
        }
        ...
        OHOS_LOGI("Sensor Accelerometer is disable Listen");
    }
    
    static void dispatchAccelerometer(Sensor_Event *event) {
        if (event == nullptr) {
            OHOS_LOGI("dispatchAccelerometer event is null");
            return;
        }
        ...
        if (errCode != NativeDisplayManager_ErrorCode::DISPLAY_MANAGER_OK) {
            OHOS_LOGI("dispatchAccelerometer displayRotation error code = %{public}d", errCode);
            return;
        }
...
int Device::getDPI() {
    ...
    if (dpi == -1)
    {
        if (auto getDpi = aki::JSBind::GetJSFunction("DeviceUtils.getDpi")) {
            dpi = getDpi->Invoke<int>(); 
        }
    }
    return dpi;
}

...
double CCTextBitmap::calxStartPosition(int pAlignment, int layoutWidth, int realWidth, int textWidth) {
    ...
    if (pAlignment == TEXT_ALIGN_CENTER) {
        // Move to the leftmost part of the text, and then add the margin between the text and the actual rendering position. 
        // Note that the content drawn from the drawing moves to the left using the - position and to the right using the + position.
...
void CCTextBitmap::createCCTextBitmap(CCTextBitmap* cCTextBitmap, const char *text,  const char *pFontName, const int fontSize,
    ...
    // COLOR_FORMAT_RGBA_8888£ºEach pixel is represented by a 32-bit quantity. 8 bits indicate transparency, 
    // 8 bits indicate red, 8 bits indicate green, and 8 bits indicate blue.
    // ALPHA_FORMAT_OPAQUE£ºBitmap has no transparency
    ...
    // Uses the specified color to clear the canvas. OH_Drawing_ColorSetArgb: Converts four variables 
    // (respectively describing transparency, red, green, and blue) to a 32-bit (ARGB) variable that describes colors.

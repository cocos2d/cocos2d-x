...
void NodeLoader::parseProperties(Node * pNode, Node * pParent, CCBReader * ccbReader)
{
    ...
    for(int i = 0; i < propertyCount; i++) {
        ...
        CCLOG("propertyName : %s, type : %d", propertyName.c_str(), type);
...
//                Color3B color3B = this->parsePropTypeColor3(pNode, pParent, ccbReader, propertyName.c_str());
            ...
            case CCBReader::PropertyType::STRING:
            {
                ...
                if( localized ) {
                    // something to do for localization
                }
...
Vector2 NodeLoader::parsePropTypePosition(Node * pNode, Node * pParent, CCBReader * ccbReader, const char *pPropertyName)
{
    ...
    Vector2 pt = getAbsolutePosition( Vector2( x, y ),
                                     (CCBReader::PositionUnit)xUnit,
                                     (CCBReader::PositionUnit)yUnit,
                                     (CCBReader::PositionType)corner,
                                     containerSize,
                                     pPropertyName);
...
Size NodeLoader::parsePropTypeSize(Node * pNode, Node * pParent, CCBReader * ccbReader) {
    ...
    CCBReader::SizeUnit widthUnit = (CCBReader::SizeUnit)ccbReader->readByte();
    CCBReader::SizeUnit heightUnit = (CCBReader::SizeUnit)ccbReader->readByte();
    ...
    // Width
    switch( widthUnit ) {
        case CCBReader::SizeUnit::SizeUnitPoints:
            size.width = width;
            break;
            
        case CCBReader::SizeUnit::SizeUnitUIPoints:
            size.width = CC_CONTENT_SCALE_FACTOR() * width;
            break;
            
        case CCBReader::SizeUnit::SizeUnitNormalized:
            size.width = width * containerSize.width;
            break;
            
        case CCBReader::SizeUnit::SizeUnitInsetPoints:
            size.width = containerSize.width - width;
            break;
            
        case CCBReader::SizeUnit::SizeUnitInsetUIPoints:
...
SpriteFrame * NodeLoader::parsePropTypeSpriteFrame(Node * pNode, Node * pParent, CCBReader * ccbReader, const char *pPropertyName) 
{
    ...
    std::string filepath = ccbReader->readCachedString();
...
Color4F NodeLoader::parsePropTypeColor4(Node * pNode, Node * pParent, CCBReader * ccbReader, const char *pPropertyName) {
...
//    int controlEvents = ccbReader->readInt(false);
    Control::EventType controlEvents = Control::EventType::TOUCH_DOWN;
...
Node * NodeLoader::parsePropTypeCCBFile(Node * pNode, Node * pParent, CCBReader * ccbReader) {
    ...
    CCBReader * reader = new CCBReader(ccbReader);
    reader->autorelease();
    ...
    CC_SAFE_RETAIN(ccbReader->_owner);
    reader->_owner = ccbReader->_owner;
    
    reader->getAnimationManager()->_owner = reader->_owner;

    // The assignments below are done in the CCBReader constructor.
//     reader->_ownerOutletNames = ccbReader->_ownerOutletNames;
//     reader->_ownerOutletNodes = ccbReader->_ownerOutletNodes;
//     reader->_ownerOutletNodes->retain();
//     reader->_ownerCallbackNames = ccbReader->_ownerCallbackNames;
//     reader->_ownerCallbackNodes = ccbReader->_ownerCallbackNodes;
//     reader->_ownerCallbackNodes->retain();

    
    Node * ccbFileNode = reader->readFileWithCleanUp(false, ccbReader->getAnimationManagers());
    ...
    if (reader->isJSControlled() && ccbReader->isJSControlled() && NULL == reader->_owner)
    {
        ...
        if (!ownerCallbackNames.empty() && !ownerCallbackNodes.empty())
        {
            ...
            {
                ccbReader->addOwnerCallbackName(ownerCallbackNames[i].asString());
                ccbReader->addOwnerCallbackNode(ownerCallbackNodes.at(i));
            }
        }
        //set variables
        ...
        if (!ownerOutletNames.empty() && !ownerOutletNodes.empty())
        {
            ...
            for (ssize_t i = 0 ; i < nCount; i++)
            {
                ccbReader->addOwnerOutletName(ownerOutletNames.at(i).asString());
                ccbReader->addOwnerOutletNode(ownerOutletNodes.at(i));
            }
        }
    }
    return ccbFileNode;
...
void NodeLoader::onHandlePropTypeColor4(Node * pNode, Node * pParent, const char* pPropertyName, Color4F pColor4F, CCBReader * ccbReader) {
    ...
    if( rgbaNode != nullptr )
	{
		...
		rgbaNode->setColor( Color3B( r, g, b ) );

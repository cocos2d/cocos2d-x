...
void S_SimpleAudioEngine::jsCreateClass(JSContext *cx, JSObject *globalObj, const char *name)
{
	...
	jsClass->flags = JSCLASS_HAS_PRIVATE;
		...
		static JSFunctionSpec st_funcs[] = {
			...
			JS_FN("setResource", S_SimpleAudioEngine::jssetResource, 1, JSPROP_PERMANENT | JSPROP_SHARED),
			JS_FS_END
		};
...
JSBool S_SimpleAudioEngine::jssetResource(JSContext *cx, uint32_t argc, jsval *vp) {
	if (argc == 1) {
		JSString *arg0;
		JS_ConvertArguments(cx, 1, JS_ARGV(cx, vp), "S", &arg0);
		char *narg0 = JS_EncodeString(cx, arg0);
		SimpleAudioEngine::setResource(narg0);

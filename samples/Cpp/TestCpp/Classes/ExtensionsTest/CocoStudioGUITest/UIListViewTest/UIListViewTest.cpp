...
bool UIListViewTest_Vertical::init()
{
    if (UIScene::init())
    {
        ...
        alert->setText("ListViewEx vertical");
        ...
        for (int i = 0; i < 20; ++i)
        {
            CCString* ccstr = CCString::createWithFormat("listviewex_item_%d", i);
...
void UIListViewTest_Vertical::selectedItemEvent(CCObject *pSender, ScrollviewEventType type)
{
    switch (type)
    {
...
bool UIListViewTest_Horizontal::init()
{
    if (UIScene::init())
    {
        ...
        UILabel* alert = UILabel::create();
        alert->setText("ListViewEx horizontal");
        alert->setFontName("Marker Felt");
        alert->setFontSize(30);
        alert->setColor(ccc3(159, 168, 176));
        alert->setPosition(ccp(widgetSize.width / 2.0f, widgetSize.height / 2.0f - alert->getSize().height * 3.075));
...
void UIListViewTest_Horizontal::selectedItemEvent(CCObject *pSender, ScrollviewEventType type)
{
    switch (type)
    {
        case LISTVIEW_ONSELECTEDITEM:

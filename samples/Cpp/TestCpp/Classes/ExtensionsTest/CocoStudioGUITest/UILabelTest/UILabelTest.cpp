...
bool UILabelTest::init()
{
    if (UIScene::init())
    {
        ...
        /*
        UILabel* cloneLabel = static_cast<UILabel*>(label->clone());
        cloneLabel->setPosition(ccp(widgetSize.width / 2, widgetSize.height / 2 - label->getSize().height / 2));
        m_pUiLayer->addWidget(cloneLabel);
         */

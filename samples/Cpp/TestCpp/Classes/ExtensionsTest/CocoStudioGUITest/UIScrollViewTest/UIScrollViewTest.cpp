...
/*
// UIScrollViewTest_TweakToPosition

UIScrollViewTest_TweakToPosition::UIScrollViewTest_TweakToPosition()
: m_pDisplayValueLabel(NULL)
{
}
...
bool UIScrollViewTest_TweakToPosition::init()
{
    if (UIScene::init())
    {
        CCSize widgetSize = m_pWidget->getSize();
        ...
        // Add the alert
        UILabel *alert = UILabel::create();
        alert->setText("ScrollView tweak to position");
        alert->setFontName("Marker Felt");
        alert->setFontSize(30);
        alert->setColor(ccc3(159, 168, 176));
        alert->setPosition(ccp(widgetSize.width / 2.0f, widgetSize.height / 2.0f - alert->getSize().height * 3.075));
        ...
        scrollView->setPosition(ccp((widgetSize.width - backgroundSize.width) / 2 +
                                    ...
                                    (widgetSize.height - backgroundSize.height) / 2 +
                                    (backgroundSize.height - scrollView->getSize().height) / 2));
        scrollView->addEventListenerScrollView(this, scrollvieweventselector(UIScrollViewTest_TweakToPosition::scrollOverEvent));
        m_pUiLayer->addWidget(scrollView);
        
        UIButton* button_scale9 = UIButton::create();
        button_scale9->setScale9Enabled(true);
        button_scale9->loadTextures("cocosgui/button.png", "cocosgui/buttonHighlighted.png", "");
        button_scale9->setSize(CCSizeMake(100, scrollView->getSize().height / 3));
        
        float innerWidth = scrollView->getSize().width;
        float innerHeight = button_scale9->getSize().height * 10;
        ...
        for (int i = 0; i < 10; ++i)
        {
            ...
            button_scale9->loadTextures("cocosgui/button.png", "cocosgui/buttonHighlighted.png", "");
            button_scale9->setSize(CCSizeMake(100, scrollView->getSize().height / 3));
            button_scale9->setTitleText(CCString::createWithFormat("cell_%d", i)->getCString());
            
            CCSize cs = button_scale9->getSize();
            CCPoint ap = button_scale9->getAnchorPoint();
            button_scale9->setPosition(ccp((innerWidth - cs.width) / 2 + (ap.x * cs.width),
                                           (innerHeight - cs.height) - (i * cs.height) + (ap.y * cs.height)));
...
void UIScrollViewTest_TweakToPosition::scrollOverEvent(CCObject *pSender, ScrollviewEventType type)
{
    if (type == SCROLLVIEW_EVENT_SCROLL_OVER)
    {
        UIScrollView* scrollView = static_cast<UIScrollView*>(pSender);
        UILayout* inner = scrollView->getInnerContainer();
        CCSize innerSize = inner->getSize();
        CCPoint innerTopPos = ccp(0.0f, inner->getTopInParent());
        CCPoint inner_to_scroll = ccpSub(innerTopPos, ccp(0.0f, scrollView->getSize().height));
        UIWidget* widget = static_cast<UIWidget*>(scrollView->getChildren()->objectAtIndex(0));
        CCSize widgetSize = widget->getSize();
        
        int offsetCell = 0;
        float totalCellDistance = 0.0f;
        float cellDelta = 0.0f;
        
        offsetCell = ceilf(inner_to_scroll.y / widgetSize.height);
        totalCellDistance = widgetSize.height * offsetCell;
        cellDelta = totalCellDistance - inner_to_scroll.y;
        
        inner->runAction(CCMoveBy::create(1.0f, ccp(0.0f, cellDelta)));
    }
}
 */
/*=*/

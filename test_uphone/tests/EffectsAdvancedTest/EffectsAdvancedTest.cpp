...
void Effect1::onEnter()
{
	...
	CCIntervalAction* delay = CCDelayTime::actionWithDuration(8);

	CCIntervalAction* orbit = CCOrbitCamera::actionWithDuration(5, 1, 2, 0, 180, 0, -90);
	CCIntervalAction* orbit_back = orbit->reverse();

	target->runAction( CCRepeatForever::actionWithAction( (CCIntervalAction *)(CCSequence::actions( orbit, orbit_back, NULL) ) ) );
...
void Effect2::onEnter()
{
	...
	CCIntervalAction* shaky = CCShakyTiles3D::actionWithRange(4, false, ccg(15,10), 5);
	CCIntervalAction* shuffle = CCShuffleTiles::actionWithSeed(0, ccg(15,10), 3);
	CCIntervalAction* turnoff = CCTurnOffTiles::actionWithSeed(0, ccg(15,10), 3);
	...
	CCIntervalAction* delay = CCDelayTime::actionWithDuration(1);
...
//	[target runAction: [RepeatForever::actionWithAction: [Sequence actions: orbit, orbit_back, nil]]];
	target->runAction( (CCIntervalAction *)(CCSequence::actions( shaky, delay, reuse, shuffle, delay->copy()->autorelease(), turnoff, turnon, NULL) ) );
}
...
void Effect3::onEnter()
{
	...
	CCIntervalAction* waves = CCWaves::actionWithWaves(5, 20, true, false, ccg(15,10), 5);
	CCIntervalAction* shaky = CCShaky3D::actionWithRange(4, false, ccg(15,10), 5);
	
	target1->runAction( CCRepeatForever::actionWithAction( waves ) );
	target2->runAction( CCRepeatForever::actionWithAction( shaky ) );
	
	// moving background. Testing issue #244
	CCIntervalAction* move = CCMoveBy::actionWithDuration(3, ccp(200,0) );
	bg->runAction(CCRepeatForever::actionWithAction( (CCIntervalAction *)(CCSequence::actions(move, move->reverse(), NULL) ) ) );	
}
...
void Effect4::onEnter()
{
	...
	CCIntervalAction* lens = CCLens3D::actionWithPosition(ccp(100,180), 150, ccg(32,24), 10);
...
void Effect5::onEnter()
{
	...
	CCIntervalAction* stopEffect = (CCIntervalAction *)( CCSequence::actions(
										 effect,
...
void Issue631::onEnter()
{
	...
	CCIntervalAction* effect = (CCIntervalAction*)(CCSequence::actions( CCDelayTime::actionWithDuration(2.0f), CCShaky3D::actionWithRange(16, false, ccg(5, 5), 5.0f), NULL));
